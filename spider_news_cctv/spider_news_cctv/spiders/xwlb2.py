...
class Xwlb2Spider(scrapy.Spider):
    name = "xwlb2"
    ...
    URL_TEMPLATE = 'http://news.cntv.cn/program/xwlb/%s.shtml'
    day = timedelta(1)
    now = date(2010, 6, 12)
    ...
    def parse_news(self, response):
        ...
        try:
            article = soup.find(class_='text_box padd').text.strip()
        except:
    ...
    def parse(self, response):
        ...
        if (str_now == '20100505'):
            ...
            if self.now != (date(2010, 6, 12) - self.day):
                try:
                    response = response.body
                    soup = BeautifulSoup(response)
