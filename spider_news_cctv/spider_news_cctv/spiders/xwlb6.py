...
class Xwlb6Spider(scrapy.Spider):
    name = "xwlb6"
    ...
    def parse_news(self, response):
        ...
        try:
            article = soup.find(class_=re.compile('line3|large')).text.strip()
        except:
    ...
    def parse(self, response):
        ...
        if (str_now == '20020907'):
            ...
            if self.now != (date(2005, 6, 8) - self.day):
                try:
                    # response = response.body
                    # soup = BeautifulSoup(response)
                    # items = soup.find(class_=re.compile('title_list_box')).find_all("li")
                    items = response.selector.xpath('//a[contains(@href, "/news/xwlb/")]').extract()
                    for i in range(1, len(items)):
                        soup = BeautifulSoup(items[i])
                        item_url = "http://www.cctv.com" + soup.a['href']
