...
class Xwlb5Spider(scrapy.Spider):
    name = "xwlb5"
    allowed_domains = ["www.cctv.com"]
    start_urls = (
        'http://www.cctv.com/news/xwlb/20061012/index.shtml',
    )
    ...
    def parse_news(self, response):
        ...
        try:
            article = soup.find(id='content').text.strip()
        except:
    ...
    def parse(self, response):
        ...
        if self.now == date(2007, 8, 14):
            self.now = self.now - self.day
        next_parse = []
        if (str_now == '20050608'):
            pass
        else:
            if self.now != (date(2007, 8, 30) - self.day):
                try:
                    # response = response.body
                    # soup = BeautifulSoup(response)
